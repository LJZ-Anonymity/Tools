...
包含 Android Debug Bridge (ADB) 工具的核心文件：

- `adb.exe` - ADB 可执行文件，用于与 Android 设备通信
- `AdbWinApi.dll` - ADB Windows API 动态链接库
- `AdbWinUsbApi.dll` - ADB USB 通信 API 动态链接库

这些文件是工具与 Android 设备通信的基础，**请勿删除或移动**。

### usb_driver 文件夹

包含 Windows USB 驱动程序，用于在 Windows 系统上识别和连接 Android 设备：
...
**首次连接设备时**，如果 Windows 无法自动识别设备，需要手动安装此驱动：
...
各文件功能参考：https://github.com/Suto-Commune/soul_knight/
...
1. ⚠️ **需要 root 权限**：还原操作需要设备具有 root 权限
2. ⚠️ **数据安全**：备份和还原操作会修改游戏数据，请谨慎操作
3. ⚠️ **备份建议**：在进行还原操作前，建议先备份当前数据
4. ⚠️ **文件路径**：不要修改或移动 `ADB Files` 文件夹中的文件
5. ⚠️ **驱动安装**：首次使用需要安装 USB 驱动，否则无法识别设备
